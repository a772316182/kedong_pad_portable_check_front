...
const policies = reactive([
  ...
  { name: "必选项核查", updated: "2025-04-18", items: 8, object: "全部设备", link: "RequiredCheck" },
...
<template>
  ...
  <q-layout view="lHh Lpr lFf" class="bg-grey-10">
    <q-header elevated class="bg-grey-9 text-white">
      <q-toolbar>
        <q-btn label = "返回" @click="navigateTo('/mainview')" color="primary"/>
        <q-toolbar-title>
          策略管理
        </q-toolbar-title>
        <q-space />
    ...
    <!-- 右侧筛选抽屉 -->
    <q-drawer v-model="isFilterDrawerVisible" side="right" overlay behavior="mobile" bordered :width="300" class="bg-grey-9">
      <q-scroll-area class="fit">
        <div class="q-pa-md">
          <h5 class="q-mt-none q-mb-md">筛选</h5>
          <!-- 筛选表单内容可以放在这里 -->
          <q-btn color="primary" label="应用筛选" class="full-width q-mt-md" @click="isFilterDrawerVisible = false" />
        </div>
      </q-scroll-area>
    </q-drawer>
      ...
      <q-page class="q-pa-md">
        ...
        <div class="row q-col-gutter-md">
          <div v-for="policy in policies" :key="policy.name" class="col-12 col-sm-6 col-md-4 col-lg-3">
            <q-card @click="handleCardClick(policy)" class="cursor-pointer full-height bg-grey-9 text-white" flat bordered v-ripple>
              <q-card-section>
