  ...
  <q-layout view="lHh Lpr lFf" class="page-background">
    <q-page class="main-content-area">
      <div class="row items-center no-wrap top-bar q-mb-md">
        <q-btn unelevated label="返回" @click="navigateTo('/stationview')" class="action-button" icon="arrow_back" size="xl"/>
      ...
      <div v-if="viewMode === 'card'">
        <q-card class="info-panel q-pa-lg q-mb-lg" flat>
          <div class="row items-center justify-between">
            <div class="column q-gutter-y-sm">
              <span class="text-h5">站点：四川省调</span>
        ...
        <q-list class="q-gutter-y-md">
          <q-expansion-item
            ...
            :key="asset.id"
            class="asset-expansion-item"
            :class="{'selected-item': selectedAssetsIds.includes(asset.id)}"
            header-class="asset-expansion-header"
            expand-icon-class="text-white"
          >
            <template v-slot:header>
              <q-item-section avatar>
                <q-checkbox :model-value="selectedAssetsIds.includes(asset.id)" @update:model-value="toggleSelection(asset.id)" color="cyan" />
              </q-item-section>
      ...
      <div v-if="viewMode === 'list'">
        <q-card class="info-panel q-pa-md q-mb-lg" flat>
          <div class="row items-center justify-between">
            <span class="text-h5">当前站点：四川省调</span>
    ...
    <q-dialog v-model="isDetailDialogOpen">
      <q-card v-if="currentDetailAsset" style="width: 600px; max-width: 90vw;">
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h4">资产详情</div>
          <q-space />
          <q-btn icon="close" flat round dense v-close-popup />
        </q-card-section>
        <q-card-section class="q-pa-lg">
          <q-form class="q-gutter-md">
            <div class="row items-center"><span class="form-label">名称：</span><q-input readonly outlined dense :model-value="currentDetailAsset.name" class="text-h5 col"/></div>
            <div class="row items-center"><span class="form-label">类型：</span><q-input readonly outlined dense :model-value="currentDetailAsset.type" class="text-h5 col"/></div>
            <div class="row items-center"><span class="form-label">最近核查：</span><q-input readonly outlined dense :model-value="currentDetailAsset.lastCheck" class="text-h5 col"/></div>
            <div class="row items-center"><span class="form-label">安全分区：</span><q-input readonly outlined dense :model-value="currentDetailAsset.securityZone" class="text-h5 col"/></div>
            <div class="row items-center"><span class="form-label">IP：</span><q-input readonly outlined dense :model-value="currentDetailAsset.ip" class="text-h5 col"/></div>
            <div class="row items-center"><span class="form-label">MAC地址：</span><q-input readonly outlined dense :model-value="currentDetailAsset.mac" class="text-h5 col"/></div>
            <div class="row items-center"><span class="form-label">系统名称：</span><q-input readonly outlined dense :model-value="currentDetailAsset.osName" class="text-h5 col"/></div>
            <div class="row items-center"><span class="form-label">系统版本号：</span><q-input readonly outlined dense :model-value="currentDetailAsset.osVersion" class="text-h5 col"/></div>
          </q-form>
        </q-card-section>
      </q-card>
    </q-dialog>
...
const navigateTo = (path) => {
  // In a real project, you would uncomment the next line
  router.push(path)
...
// Updated to work with the checkbox or any toggle mechanism
...
const deleteSelectedAssets = () => {
  ...
  // Using $q.dialog for a more integrated look
  $q.dialog({
    title: '确认删除',
...
/* --- NEW Accordion/Expansion View Styles --- */
.asset-expansion-item {
...
.asset-expansion-item.selected-item {
  border-color: #4c6afc;
  box-shadow: 0 0 10px rgba(76, 106, 252, 0.5);
}
.asset-expansion-header {
  color: #e0e0e0;
  padding: 8px 16px;
  min-height: 72px; /* Ensure consistent header height */
}
.expansion-content-card {
  background-color: #3d4a58; /* Match the parent color */
  color: #e0e0e0;
}
...
/* Quasar Override for dark dialogs */
